...
abstract class ApiService {
  ...
  /// Authenticate with email + password. Returns a token pair only — the
  /// user profile is fetched separately via [getMe].
  @POST('auth/login')
  Future<ApiResult<LoginResDm>> login(@Body() LoginReqDm body);
  ...
  // ---------------------------------------------------------------------
  // Mobile / employee (unchanged — mocked on the mobile side)
  // ---------------------------------------------------------------------
  ...
  /// real employee auth session exists.
  @GET('me/devices')
